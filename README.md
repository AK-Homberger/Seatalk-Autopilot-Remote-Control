...
It's usually better to use the [74LS07](https://www.ti.com/lit/ds/symlink/sn74ls07.pdf) variant of the device, because it needs less power then the original TTL 7407. The [74HC07](https://www.dxdz-ic.com/upload/pdf/DX-74HC07.pdf) and 74HCT07 variants will not work in this szenario, because they don't implement a real "Open Collector" output and output voltage is limited to VCC (5 Volt here), but we need > 12 Volt for the Seatalk bus. Unfortunately, it looks like that some devices sold as 74LS07 are CMOS based equivalents with a diode from VCC to output that limits to 5 Volt.
